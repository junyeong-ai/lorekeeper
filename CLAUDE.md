...
- **`lore schedule` emits cron OR launchd** (`--format`). On macOS launchd is correct: `StartCalendarInterval` runs a job missed during sleep as soon as the machine wakes, whereas cron silently skips it — and a closed laptop at 09:00 is the normal case. Cron syntax launchd cannot express (`*/5`, `1-5`) is REFUSED, never approximated, since a silently different schedule is worse than a rejected one; `--bin` must be absolute because launchd does not search `PATH`.
