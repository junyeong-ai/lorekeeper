...
- **Atomic ingest**: plan → write daily/concept → work-log → flush LLM queue → commit dedup, then a post-commit archive hook for the `manual` source. Each source's dedup commits only after its own writes + flush succeed; the archive runs only after a successful commit, so a mid-run failure leaves inbox files for retry.
