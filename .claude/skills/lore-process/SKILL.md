...
The queue file lifecycle (`*.jsonl.tmp` → `*.jsonl` → `processed/`), the SIGSTOP
crash-recovery path, and the full per-line task JSON schema (the `kind` /
`target.kind` enumerations, `cache_hash`, `target.anchor`) are read-once reference
— see [references/queue-format.md](references/queue-format.md). The essentials the
protocol below relies on:

- A visible `.jsonl` is fully written and every `target.vault_path` already exists
  (atomic temp + fsync + rename). Only `.jsonl` files (not `.tmp`) are for consumption.
- Locate each section by `target.anchor` (the exact heading), never by hardcoding a
  heading per `target.kind`.
- `cache_hash` equals the page's `llm_inputs.<key>` at queue time; verify it matches
  before writing — see the Stale-task guard below.
