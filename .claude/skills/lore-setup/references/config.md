...
sources:                             # 키 = 소스 id = daily/{id}/ 디렉토리명
  <id>:
    ...
    enabled: true
    schedule: "0 9 * * 1-5"          # cron (lore schedule이 사용)
...
## 위치 / 검증
- `./config.yaml`(repo) 또는 `~/.config/lorekeeper/config.yaml`(바이너리 설치).
  `--config`/`LORE_CONFIG`로도 지정. vault 하위에는 둘 수 없다(vault 경로가 config 안에 있어 순환).
- 작성 후 항상 `lore validate` → `lore ingest --dry-run`으로 확인.
